...
WHERE mc2.curso_id = :curso2
AND EXISTS (
    SELECT 1 
    FROM matrizcurricular mc1
    WHERE mc1.codigo_id = mc2.codigo_id
    AND mc1.curso_id = :curso1
);
...
FROM historico h
JOIN disciplina d ON h.codigo = d.codigo_disciplina
JOIN professor p ON d.codigo_disciplina = p.id_prof
WHERE h.ra_aluno = :ra_aluno;
